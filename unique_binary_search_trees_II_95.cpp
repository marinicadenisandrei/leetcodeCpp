...
vector<vector <int>> generateTrees(int nVar);
...
int main()
{
    ...
    cout << "Leetcode - 95. Unique Binary Search Trees II (C++ language) - Medium" << endl;
    ...
    for (int test = 0; test < n.size(); test++)
    {
        ...
        vector<vector <int>> output = generateTrees(n[test]);
        printMatrixVector(output);
